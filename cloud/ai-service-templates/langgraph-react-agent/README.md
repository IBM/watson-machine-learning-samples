...
```sh
git clone --no-tags --depth 1 --single-branch --filter=tree:0 --sparse https://github.com/IBM/watson-machine-learning-samples.git
...
```sh
cd cloud/ai-service-templates/langgraph-react-agent
```
...
Running the below commands will install the repository in a separate virtual environment. We recommend to use a virtual env manager, for example `pipx` or `uv`:
...
You can use any other tool to run your Python code, make sure to use Python v10 to v13.
...
- `watsonx_url`: Your watsonx.ai API URL, for example `https://us-south.ml.cloud.ibm.com`
- `space_id`: The ID of the space where the agent templae will be deployed (must have an active watsonx.ai runtime attached)

Go to the [Developer Access](https://dataplatform.cloud.ibm.com/developer-access) page to find your environment variables.

### Step 5: Run the script for local AI service execution  
