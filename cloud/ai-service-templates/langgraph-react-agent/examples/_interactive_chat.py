...
class InteractiveChat:
    def __init__(self, ai_service_invoke: Callable, questions: tuple[str] = None, data: dict = None,
                 stream: bool = False, verbose: bool = True) -> None:
    ...
    def _print_message(self, message: dict) -> None:
        print(f" {message['role'].capitalize()} Message ".center(80, '='))
                ...
                for action, stage in user_loop:  # unsupported command support!
                    ...
                    if action == "h" or action == "help":
                        ...
                        if not action.isdigit():  # user defined question
                            request_payload_json["question"] = action.strip()
                        else:
                            number = int(action)
                            ...
                            if number > len(self.questions) or number < 0:
                                ...
                                request_payload_json["question"] = self.questions[number - 1]
                                request_payload_json["data"] = {
                                    "exog": self.data[self._data_names[d - 1]].load_pandas().exog.iloc[:25, 0].to_numpy().tolist(),
                                    "endog": self.data[self._data_names[d - 1]].load_pandas().endog.iloc[:25].to_numpy().tolist(),
                                }
